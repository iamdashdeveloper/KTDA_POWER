...
import { loadProjectFeatures, loadProjectIssues, parseGeometry, loadFeatureById } from '@/lib/mapData';
import { getProjectCoordinates } from '@/components/maps/mapUtils';
import { featureStyleFunction, issueStyleFunction } from '@/components/maps/mapStyles';
...
export const OpenLayersMap: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    const features = new VectorLayer({ source: featuresSource, style: featureStyleFunction, zIndex: 10 });
    const issuesSource = new VectorSource();
    const issues = new VectorLayer({ source: issuesSource, style: issueStyleFunction, zIndex: 20 });
    ...
    map.on('singleclick', async (event) => {
      ...
      if (isIssueLayer) {
        ...
        try {
          ...
          openPanel('right', <IdentifyPanel key={issueId} data={properties} type="issue" />, 'Issue Details');
          setCollapsed('right', false);
        }
        ...
        openPanel(
          'right', 
          <IdentifyPanel key={properties.id} data={properties} type="feature" />, 
          'Feature Details'
        );
        setCollapsed('right', false);
      }
  ...
  React.useEffect(() => {
    ...
    layers.forEach(layer => {
      switch (layer.id) {
        case 'osm': layersRef.current!.osm.setVisible(layer.visible); break;
        case 'satellite': layersRef.current!.satellite.setVisible(layer.visible); break;
        case 'reference': layersRef.current!.reference.setVisible(layer.visible); break;
        case 'project-features': layersRef.current!.features.setVisible(layer.visible); break;
        case 'project-issues': layersRef.current!.issues.setVisible(layer.visible); break;
      }
    });
  }, [layers]);
  ...
  React.useEffect(() => {
    ...
    const loadScratch = async () => {
      ...
      try {
        ...
        setScratchFeatures(allFeatures.map(f => ({
          id: f.id,
          name: f.name,
          visible: true,
  ...
  // Load Project Data
  React.useEffect(() => {
    if (!activeProject?.id || !mapInstanceRef.current || !layersRef.current) return;
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [featuresData, issuesData, scratchFeaturesRaw] = await Promise.all([
          loadProjectFeatures(activeProject.id),
          loadProjectIssues(activeProject.id),
          Promise.all(scratchIds.map((id: string) => loadFeatureById(id).catch(() => null)))
        ]);
        ...
        setStats(featuresData.length + scratchFeaturesData.length, issuesData.length);
        setProjectFeatures(featuresData.map(f => ({
          id: f.id,
          name: f.name,
          visible: true,
        ...
        setScratchFeatures(scratchFeaturesData.map(f => ({
          id: f.id,
          name: f.name,
          visible: true,
        ...
        const geojsonFeatures = [...featuresData, ...scratchFeaturesData].map(f => {
        ...
        const issueFeatures = issuesData.map(issue => {
          ...
          if (issue.location && typeof issue.location === 'object') {
             ...
             if (geom.type === 'Point' && Array.isArray(geom.coordinates)) {
               coords = [Number(geom.coordinates[0]), Number(geom.coordinates[1])];
             }
          }
        ...
        if (projectCoords) {
          ...
          mapInstanceRef.current?.getView().fit(featureSource.getExtent(), {
            padding: [50, 50, 50, 50],
            duration: 1000
          });
        }
  ...
  React.useEffect(() => {
    ...
    switch (id) {
      ...
      case 'full-extent':
        ...
        if (extent) view.fit(extent, { duration: 1000, padding: [50, 50, 50, 50] });
        break;
      case 'reset-north':
        view.animate({ rotation: 0, duration: 500 });
        break;
      case 'zoom-to-layer': {
        const { id: layerId, name: layerName, type: layerType } = currentCommand.payload || {};
        if (!layerId) break;

        let zoomExtent = createEmpty();

        if (layerType === 'project' || layerType === 'scratch') {
          const features = layersRef.current?.features.getSource()?.getFeatures() || [];
          const matchingFeatures = features.filter(f => {
            const props = f.getProperties();
            const cleanId = layerId.replace('group-', '').replace('scratch-', '');
            return (props.groupName === layerName) || (props.groupId === cleanId) || (props.parentId === cleanId);
          });
          ...
          const extent = (layersRef.current as any)[layerId].getSource()?.getExtent();
          if (extent) zoomExtent = extent;
        }
  ...
  const handleFullExtent = () => {
    const extent = layersRef.current?.features.getSource()?.getExtent();
    if (extent) mapInstanceRef.current?.getView().fit(extent, { duration: 500, padding: [50,50,50,50] });
  };

  return (
    <div className="relative w-full h-full bg-muted/10 overflow-hidden group">
      ...
      {error && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-50">
          <div className="bg-destructive/10 border border-destructive/20 text-destructive text-[11px] px-4 py-2 rounded-full flex items-center gap-2 backdrop-blur-md shadow-lg">
            <AlertCircle size={14} />
            {error}
          </div>
        </div>
      ...
      <div className="absolute bottom-8 left-8 z-20 pointer-events-none">
        <div className="bg-card/80 backdrop-blur p-2 rounded shadow-sm border border-border">
      ...
      {mapInstanceRef.current && (
        <>
          <ZoomBoxLogic map={mapInstanceRef.current} active={activeTool === 'zoom-box'} />
