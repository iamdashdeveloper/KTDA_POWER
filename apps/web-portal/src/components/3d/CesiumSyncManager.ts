...
export class CesiumSyncManager {
   ...
   */
  public initialize(targetElement: HTMLDivElement): OLCesium {
    ...
    (this.olcs as any).enableAutoRenderLoop();
    
    // Enable shadows for vector layers
    this.olMap.getLayers().forEach(layer => {
        if (layer instanceof (window as any).ol?.layer?.Vector || (layer as any).getSource?.()?.getFeatures) {
            layer.set('olcs_shadows', true);
            console.log(`[CesiumSyncManager] Enabled shadows for vector layer: ${layer.get('name') || 'unnamed'}`);
        }
    });

    console.log(`[CesiumSyncManager] Synchronizing layers to 3D. Map has ${this.olMap.getLayers().getLength()} layers.`);
  ...
  private setupVectorStyling() {
    ...
    scene.preRender.addEventListener(() => {
      ...
      const length = typeof dataSources.length === 'number' ? dataSources.length : (dataSources.count ? dataSources.count() : 0);
  ...
  private configureScene(scene: Cesium.Scene) {
    ...
    Cesium.createWorldTerrainAsync().then((terrainProvider) => {
      ...
      console.warn('[CesiumSyncManager] World terrain unavailable, using ellipsoid fallback')
    })
