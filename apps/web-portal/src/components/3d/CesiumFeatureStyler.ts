...
export class CesiumFeatureStyler {
  ...
  private static stylePolygon(entity: Cesium.Entity, feature: Feature) {
    ...
    const color =
      entity.polygon.material?.getValue(Cesium.JulianDate.now())?.color ||
      Cesium.Color.CYAN
  ...
  private static styleLine(entity: Cesium.Entity, feature: Feature) {
    ...
    const lineHeight = feature.get("elevation") || feature.get("height") || 5

    // Use lineHeight for potential elevation adjustments
    void lineHeight
