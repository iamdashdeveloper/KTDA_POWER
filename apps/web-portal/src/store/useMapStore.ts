...
export const useMapStore = create<MapStore>((set) => ({
  layers: [
    ...
    { id: 'reference', name: 'Reference Labels', visible: true, type: 'reference' },
    { id: 'project-features', name: 'Project Features', visible: true, type: 'feature' },
  ...
  viewZoom: 12,
  setViewCenterZoom: (center, zoom) => set({ viewCenter: center, viewZoom: zoom }),
  currentCommand: null,
  executeCommand: (id, payload) => set({ currentCommand: { id, timestamp: Date.now(), payload } }),
  terrainCommand: null,
  executeTerrainCommand: (id) => set({ terrainCommand: { id, timestamp: Date.now() } }),
  projectFeatures: [],
  scratchFeatures: [],
  hiddenFeatureIds: new Set(),
  setProjectFeatures: (features) => set({ projectFeatures: features, hiddenFeatureIds: new Set() }),
  setScratchFeatures: (features) => set({ scratchFeatures: features }),
  addScratchFeature: (feature) => set((state) => ({ 
    scratchFeatures: [...state.scratchFeatures, feature] 
  })),
  removeScratchFeature: (id) => set((state) => ({
  ...
  toggleGroupVisibility: (groupName) => set((state) => {
    const groupFeatures = state.projectFeatures.filter(f => (f.groupName || 'Other Features') === groupName);
  ...
  featureCount: 0,
  issueCount: 0,
  setStats: (features, issues) => set({ featureCount: features, issueCount: issues }),
  refreshTrigger: 0,
